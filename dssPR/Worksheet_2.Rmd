...
Going forward, the worksheets will assume that you've followed the steps above, and that you place all new code inside the `code` subfolder and all new data sets inside the `data` subfolder. Overall, this will save a lot of hassle, and make it much easier to work with real data sets, especially as the number of code and data files increases. If however the above steps did not work or you prefer working without an R Project, you can always load the data files by either writing out their full *absolute* path, or putting them in the same folder as your worksheet and writing only the data's filename as a path.
...
twokitchen <- houses[houses$Kitchen.AbvGr == 2 & houses$Kitchen.Qual == "Gd", ]
...
q10_houses <- q10_houses |> mutate(Price.Area_dplyr = SalePrice / Gr.Liv.Area)
q10_houses <- q10_houses |> mutate(ModeratePrice_dplyr = SalePrice >= 129500 & SalePrice <= 213500)
```
...
bsmt <- houses |> filter(is.na(Bsmt.Qual) == FALSE)
...
housecopy <- houses |> arrange(desc(Gr.Liv.Area), SalePrice, desc(Lot.Area))
...
sales_per_month |> arrange(desc(Num.Sales))
...
housegroup <- houses |> 
  group_by(Bldg.Type) |> 
  summarise(avgRooms = mean(TotRms.AbvGrd), minRooms = min(TotRms.AbvGrd), maxRooms = max(TotRms.AbvGrd))
...
old_basements <- old_basements |> 
  arrange(desc(Total.Bsmt.SF), Bsmt.Unf.SF)
...
millennium_mansions <- houses |> 
  filter(Gr.Liv.Area > 3000, 
         Full.Bath + Bsmt.Full.Bath >= 3, 
         Roof.Style %in% c("Gable", "Hip"),
         Garage.Type == "Attchd",
         Garage.Cars >= 2
         ) |> 
  arrange(desc(Lot.Area)
          ) |> 
  select(SalePrice
         ) |> 
  slice(1:5)
