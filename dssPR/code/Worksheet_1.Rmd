...
Welcome to the course 'Research Skills: Programming with R'! You're looking at Worksheet 1. It contains both informative text and exercises, and if you work through each week's material and practice with the quizzes regularly, you should be sufficiently prepared for the final examination.
...
If you do either of these things, you should see the calculation and its result appear in the **Console** tab. Did that work? Great. In Worksheet 3, when we start drawing plots, they'll appear too, in the **Plots** tab.

In general, in this course, each .Rmd Worksheet will have exercises scattered throughout the text, as well as a set of exercises at the end of each section. They will be marked by a number in between parentheses, like so (1), and an R code chunk will be provided for you to type your answer into.
...
An object can have any name, as long as it doesn't start with a number or underscore, and doesn't contain any punctuation marks other than underscores and periods.

(1) Bearing the above in mind, create your own object and print it:

```{r Exercise 1.}
## create your own object, and print it to the console
shortstory <- "The Ents and Huorns move towards Isengard"
shortstory
```
...
(4) Look at your workspace, and then remove one of your new objects.
...
R comes with lots of built-in functions, and R packages are mostly just add-ons of extra functions. In Worksheet 4, we'll learn how to define our own functions, but for now, let's just look at an existing one:
...
When calling functions, there's three rules to remember: You must specify all arguments that don't have defaults, or risk errors. If you don't name arguments as you supply them, they will be matched to the function's arguments in order. Finally, if you do name them, you can supply them in any order you like.

```{r Specifying function inputs.}
## these calls to the 'round' function are all the same
round(3.65, digits = 1)     ## explicitly naming the 'digits' argument
round(3.65, 1)              ## no naming of arguments, so just matched in order
round(digits = 1, x = 3.65) ## both arguments named, so can reverse order
```
...
For most functions, supplying more inputs than it explicitly expects will generate an error. For example, try typing `round(3.65, 4, 1)` into the **Console**. However, some functions accept a special argument, `...`, which matches any inputs not matched by any other argument.

For example, if you look `?round()`, you will also see the function `trunc()`. This function returns its input with all the decimals removed. But right now, what's relevant is that it has a `...` argument. This means that you can call it with all the additional input that you like, and it won't complain:

```{r Round() versus trunc()., error = TRUE}
...
So why does `trunc()` accept a `...` argument? That's a bit complex to explain, but sometimes accidentally matching a `...` argument can cause very unexpected errors, so it's good to be aware of them.
...
paste("This one", "though", "has only three.")
```

As a final point, you might wonder how to find useful R functions that you don't already know the name of. The short answer is that within R there is no good way to do this. Instead, just Google what you are trying to do; usually there will be a Stackoverflow post pointing you in the right direction.

#### Exercises

(7) Knowing what you now know about functions, and R's `?` help, can you...

-   use the function `log()` to get the base 2 logarithm of 8?

-   use the function `rnorm()` to get a random number drawn from a normal distribution with a mean of 0 and a standard deviation of 3?

-   use the function `substr()` to extract the letters 'red' from 'credit'?

(Hint: Read the `?` for each function carefully; it should be possible to figure out how to supply the arguments in order to get the requested result, even if you're not sure what a logarithm is, or a normal distribution.)

```{r Exercise 7., eval = FALSE}
## find the base 2 logarithm of 8 using 'log()'
log(8, 2)
...
# first rounds then creates a character with one digit
# second rounds to one digit then converts to character
```
...
paste(paste(toupper(substr(bird_name, 1, 1))), substr(bird_name, 2, 5),". Weight:", bird_weight, weight_unit, ". Wingspan:", bird_span, span_unit, ".", sep = "")
...
The simplest R data structure is the *atomic vector*: an ordered collection of items of the same basic, or 'atomic', type. R's most common atomic types are character, numeric, and logical. You can define an R atomic vector using `c()`, where the 'c' is short for 'concatenate':
...
Now, `TRUE` becomes 1 while `FALSE` becomes 0. This can be useful, as we will see in Worksheet 2. But R's automatic conversions become especially important when reading in real data: Just one non-numeric input will cause R to convert everything to character, which can be problematic.
...
## concatenating the two 'mix_vecs'
mix_vec_3 <- ##add code
...
If you need to store information in a table-like structure, what you want is probably a `data.frame()`. A data frame is just a list of vectors of equal length, with named columns. You build one like this:
...
(11) Let's say you wanted an object that would store together the name of a zoo as a string, the types of animals it has in a vector, and last year's profit as 1 number. What data structure would you use for this? Can you produce an example, using four animals? You can build it up out of intermediate objects if you like.

```{r Exercise 11.}
## store together the name of a zoo, its animals, and its profit
list(zoo = "The Shire", animals = c("Hobbits", "Wizards", "Men", "Dwarfs"), profit = 5)
```
...
data.frame(animals = c("Hobbits", "Wizards", "Men", "Dwarfs"), totals = c(95, 1, 4, 1), height = c(1.00, 2.10, 1.80, 1.30), feed = c(4, 25, 8, 10))
```
...
(15) Let's say you really despise oranges, but love pears. In the data frame my_df, can you change the tastyness of oranges to 3 and the tastyness of pears to 9? Use two different indexing methods to do so.
...
(16) Now that you've fixed the tastyness ratings, can you add another column to the my_df data frame, and then return only row 3?

```{r Exercise 16., eval = FALSE}
## add another column to the my_df data frame, then return row 3
my_df$justbigTHREE <- c(3, 3,3)
my_df
```
...
round(my_list[[2]])
# BECAUSE IT IS A LIST RETURNED AND NOT A NUMBER BUT IT'S FIXED NOW
```
...
For now, let's just practise using vectorised functions, while also integrating skills from the previous sections.

(18) This code appears to produce the wrong result. Why? Can you fix it?

```{r Exercise 18.}
## fix this piece of code
mean_of_items <- mean(c(4, 5, 6))
mean_of_items

```

(19) In `my_df`, we have two numeric columns: 'tastyness' and 'healthyness'. Write one line of code which adds a 'goodness' colummn which is the average of the other two. Hint: Think back to Chunk 39, and remember that a column in a data frame is just a vector; how do you add up two vectors item by item?

```{r Exercise 19.}
## add a third column to the 'my_df' data frame
my_df$goodness <- my_df$tastyness + my_df$healthyness
my_df
```
...
(20) Create a data frame with 6 columns and 6 rows. From left to right, the columns should contain:
...
randomdraw <- rnorm(6, 5)
multiple <- numbers * randomdraw
wtfwhy <- c(numbers[1:3], randomdraw[1:3])
bool <- randomdraw > numbers 
cursednames <- c("5", "[]", "not()", "for", "while", "4head")


data.frame(numbers, randomdraw, multiple, wtfwhy, bool, cursednames)
```
...
13. explain what a 'vectorised' function is
