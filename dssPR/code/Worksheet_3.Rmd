...
#knitr::opts_knit$set()
## note: this bit of RMarkdown code is only required if you want to 'knit' the worksheet to HTML or PDF
```
...
A plot will either be visible under your chunk, in the Plots tab to the side, or as a pop-up window. For this worksheet, we recommend to print the plots under your chunks by setting the *Chunk Output Inline* option under the gear at the top (next to Knit) or under Global Options.

The plot already looks pretty good, but what, exactly, have we done? We've started building a plot using `ggplot2`, which is widely considered the best R package for plotting. Like last week's `dplyr`, it was developed by Hadley Wickham, and it implements his "Grammar of Graphics" philosophy.
...
houses |> ggplot(aes(x = Gr.Liv.Area, y = SalePrice, size = Lot.Area)) +
  geom_point()
```
...
# Function should be in the ggplot() call because geom_smooth looks there
```
...
houses |> 
  ggplot(aes(x = Garage.Area, fill = Garage.Type)) +
    geom_boxplot()
```
...
houses <- group_by(houses, Bldg.Type)
avg_rms <- summarise(houses, AvgRms = mean(TotRms.AbvGrd))

avg_rms |> ggplot(aes(x = Bldg.Type, y = AvgRms)) +
  geom_bar(stat = "identity")
```

(6) Create a barplot splitting `Year.Built` into 20 bins (use `?` if you're not sure how). Color the bars by `Bldg.Type`. When would you say was the "age of the Duplex"? (a rough time period is fine)

```{r Exercise 6. Create a binned barplot., fig.height = 2, fig.width = 8}
## create a *binned* barplot of number of houses per `Year.Built`
houses |> ggplot(aes(x = Year.Built, fill = Bldg.Type)) +
...
Note, however, the odd legend: Instead of showing a separate line for each year, it's showing a gradual color scale from light to dark blue. That's because `ggplot()` thinks `Yr.Sold` is an ordinary number, which can vary continuously - as if "2006.5" would be a valid entry. But it isn't! We want to use it as a variable to 'group' by.

So how do we fix this? The simplest way is to turn `Yr.Sold` into a *factor*. A *factor* is a special R type for data that can only take on a few possible values, known as *levels*. Factors are meant especially for categorical data - data that can only take on a small number of distinct values, such as a Treatment column with 2 values: Drug or Placebo. In the example above, we want `Yr.Sold` to behave like that with each year being a group (to group by in ggplot), so we convert it simply using the `factor()` function:
...
Unfortunately, now that we've turned `Yr.Sold` into a factor, we can't add new years to it or do ordinary math operations. For instance, if we try to find all the rows with `Yr.Sold` before 2008, all we'll get is rows of `NAs`; recall that `NA` is a special R value indicating `Not Available`:
...
So let's turn `Yr.Sold` back into an ordinary numeric variable, using `as.numeric()` and `as.character()`. Don't forget the `as.character()`; converting to the factor to `as.numeric()` directly gives bizarre output, for reasons that we don't have to get into. Just re-read in the data if this happens.
...
*Sidenote*: when loading in data e.g. `read.delim()`, there is an argument `stringsAsFactors` which is by default set to FALSE. As it's name says, this argument can convert columns with strings in your file immediately to factors. This argument was set to TRUE by default in older versions of R, which is why it's important you use more recent versions of R as the default is now FALSE and we want it to be FALSE for our worksheets.
...
mean_yr <- group_by(houses, Yr.Sold)
mean_yr <- summarise(mean_yr, AvgPr = mean(SalePrice))

ggplot(mean_yr, aes(x = Yr.Sold, y = AvgPr)) + 
  geom_bar(stat = "identity")
```
...
ggplot(houses, aes(x = Yr.Sold, linetype = Bldg.Type)) +
  geom_line(stat = "count")
```

(10) Create a line graph with `Garage.Yr.Built` on the x-axis and the number of garages on the y-axis; each number of `Garage.Cars` should be its own line, with a different color and in a legend. Use the `garages` subset for this; the full `houses` data set has one garage built in the future!
...
garages$Garage.Fcars <- factor(garages$Garage.Cars)

garages |> ggplot() +
  geom_smooth(aes(x = Garage.Yr.Blt, color = Garage.Fcars), stat = "count")
```
...
In the example above, we've used `scale_x_continuous()` to control the limits of the x-axis, while we've used `scale_color_discrete()` to set the color of variables with `Garage.Type` equal to `NA` to "black". Compare, for example, the plot above with this one, where we've left all *scales* at their defaults:
...
For instance, in a `scale_..._...()` function, the argument `name` specifies titles, and `breaks` specifies either tick marks (for axes) or item ordering (for legends). The argument `guide = FALSE` suppresses a legend entirely.
...
Finally, note that using `?` on a `scale_..._...()` function doesn't directly summarize it completely. You have to click through to 'other arguments passed on to...' to see most of the arguments we've just used (e.g. `guide`) and that still doesn't list all their possible values (e.g. `guide = FALSE`).
...
ggplot(data = houses, aes(x = Lot.Area, y = SalePrice, color = Gr.Liv.Area)) +
    geom_point() +
    scale_colour_gradientn(colors = c("yellow", "red"))
```
...
ggplot(houses, aes(x = House.Style)) +
  geom_bar()+
  scale_x_discrete(limits = c("1Story", "2Story", "1.5Unf", "2.5Unf", "2.5Fin"))
...
ggplot(houses, aes(x = Lot.Area, y = SalePrice, color = Bldg.Type, size = Gr.Liv.Area)) +
  geom_point() +
  theme(legend.box = "horizontal", legend.background = element_rect(fill = "darkgrey"))
```
...
houses$FactorBath = factor(houses$Full.Bath)

ggplot(houses, aes(x = Bedroom.AbvGr, fill = FactorBath)) +
  geom_bar() +
  scale_x_discrete(name = "Number of Bedrooms") +
  scale_y_discrete(name = "Number of Houses") +
  scale_fill_discrete(type = c("#C3BCB2", "#CC9933", "#339900", "#008EC6", "#003366"), name = "Number of Bathrooms") +
  theme(legend.position = "bottom") +
